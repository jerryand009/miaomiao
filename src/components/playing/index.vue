<template>
    <div class="movieList">
        <Loading v-if="loading"/>
        <Scroller :handleScroll="handleScroll" :handleScrollEnd="handleScrollEnd" v-else>
            <ul>
                <li>{{pullDownMsg}}</li>
                <li v-for="item in movieList" :key="item.id">
                    <div class="movieItem" @tap="handleTap(item.id)">
                        <div class="pic">
                            <img :src="item.img|setWidthHeight('120.80')" alt="">
                        </div>
                        <div class="info">
                            <h2>{{item.nm}}<img v-if="item.version" src="data:image/png;base64," alt=""></h2>
                            <p>观众评分&nbsp;<span>{{item.sc}}</span></p>
                            <p>{{item.star}}</p>
                            <p>{{item.showInfo}}</p>
                        </div>
                        <div class="btnBuy">
                            <input type="button" value="购票" />
                        </div>
                    </div>
                </li>
            </ul>
        </Scroller>
    </div>
</template>

<script>
    export default {
    name: "playing",
    data() {
      return {
        movieList: [],
        pullDownMsg: "",
        loading:true,
        prevCity:-1,
      };
    },
    activated() {
      let currentCity = this.$store.state.city.id;
      if (this.prevCity!=currentCity){
        this.axios.get("/api/movieOnInfoList?cityId="+currentCity).then((res) => {
          if (res.data.msg === "ok") {
            this.movieList = res.data.data.movieList;
            this.loading = false
          }
        });
        this.prevCity = currentCity;
      }

    },
    methods: {
      handleTap(movieId) {
        console.log("tap"+movieId);
        this.$router.push("/movie/detail/"+movieId)
      },
      handleScroll(position,scroll) {
        console.log('scroll',position,"__",scroll);
        if (position.y > 50) {
          this.pullDownMsg = "正在刷新";
        }else if(position.y < scroll.maxScrollY-50){
          this.pullDownMsg = "正在加载更多";
        }
      },
      handleScrollEnd(position,scroll) {
        console.log('scrollEnd',position,"__",scroll);
        if (position.y === 0 && scroll.distY>200 ) {
          console.log("发起刷新请求");
          this.axios.get("/api/movieOnInfoList?cityId=10").then((res) => {
            if (res.data.msg === "ok") {
              this.movieList = res.data.data.movieList;
              this.$nextTick(() => {
                this.pullDownMsg = "";
              });
            }
          });
        }else if (position.y === scroll.maxScrollY && scroll.distY<-200){
          console.log("加载更多");
        }
      }
    }
  };
</script>

<style scoped lang="scss">
    .movieList {
        margin: 0 10px;
        height: 100%;
        ul {
            display: flex;
            flex-wrap: wrap;
            li {
                width: 100%;
                .movieItem {
                    display: flex;
                    justify-content: space-between;
                    border-bottom: 1px solid #ccc;
                    padding-top: 10px;
                    padding-bottom: 10px;
                    .pic {
                        width: 64px;
                        img {
                            width: 100%;
                        }
                    }
                    .info {
                        flex: 1;
                        text-indent: 10px;
                        display: flex;
                        flex-direction: column;
                        h2 {
                            flex: auto;
                            font-weight: bold;
                            display: flex;
                            justify-content: space-between;
                            img {
                                width: 50px;
                                height: 16.27px;
                            }
                        }
                        p {
                            width: 200px;
                            margin-bottom: 2px;
                            color: #666;
                            font-size: 13px;
                            overflow: hidden;
                            text-overflow: ellipsis;
                            white-space: nowrap;
                            span {
                                font-weight: bold;
                                color: #faaf00;
                            }
                        }
                    }
                    .btnBuy {
                        display: flex;
                        align-items: center;
                        input {
                            width: 47px;
                            height: 27px;
                            border: none;
                            border-radius: 5px;
                            padding: 0;
                            background-color: $baseColor;
                            color: #fff;
                        }
                    }
                }
            }
        }
    }
</style>